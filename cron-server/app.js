...
cron.schedule("* * * * *", async () => {
  ...
  status.map(async (item) => {
    ...
    if (minutes <= 0) {
      ...
      message = `Halo, ini sudah 15 hari semenjak kamu terakhir kali service ${user.vehicle.name} nih, jangan lupa untuk tetap menjaga kesehatan ${user.vehicle.name} kamu, kamu juga bisa konsultasi dari menu chat yang tersedia ya!.`;
      HP = 100;
    } else if (minutes === 2) {
      message = `Halo, ini sudah 30 hari semenjak kamu terakhir kali service ${user.vehicle.name} nih, jangan lupa untuk menjadwalkan service ya untuk tetap menjaga kesehatan ${user.vehicle.name} kamu`;
      HP = 75;
    } else if (minutes === 3) {
      message = `Halo, ini sudah 45 hari semenjak kamu terakhir kali service ${user.vehicle.name} nih, segera jadwalkan service ya untuk tetap menjaga kesehatan ${user.vehicle.name} kamu`;
      HP = 50;
    } else if (minutes === 4) {
